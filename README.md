...
面向街道、社区和应急仓库的防灾物资储备与调拨平台，覆盖物资库存、避难点、事件响应和调拨审批。调拨审批为可操作流程：详情内可见来源仓库、物资批次剩余量与到期日，批准按 FEFO（临期批次先出）锁方案，库存不足保持待审并列缺口，仓库停用或跌破安全库存需说明原因；审批、出库、签收结果留在列表、详情时间线中，切换页面仍可见。
